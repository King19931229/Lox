...
### `NamedVariable`：碰到变量名时编译什么

Pratt 里 `IDENTIFIER` 的 prefix 处理函数。`var x = 1` 声明不走这里，只处理**已存在名字**的读写。

表达式约定：**栈顶是结果。**

| 源码 | 编译 | 运行后栈顶 |
|------|------|------------|
| 读 `x` | `GET_LOCAL` / `GET_UPVALUE` / `GET_GLOBAL`（按上面解析顺序选） | 有 `x` 的值 |
| 写 `x = expr` | 先编 `expr`（值上栈）→ 再 `SET_*` 把栈顶抄进 `x`（**不弹栈**） | 仍有 `expr` 的值 |

一句话：**让栈顶有这份值；若是赋值，再多一条 `SET` 写进变量。**

赋值语句 `x = 1;` 后面还有 `ExpressionStatement` 的 `OP_POP` 把栈顶丢掉；赋值**表达式** `a = b = 1` 则依赖栈顶留着。
